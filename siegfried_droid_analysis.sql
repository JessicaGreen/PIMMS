...
---Save this output as new csv file or as a view in DB Browser to query and join with folder-level analysis spreadsheets
WITH files_and_paths AS (
	SELECT FILE_PATH, 
	substr(FILE_PATH, 1, length(FILE_PATH) - length(NAME) - 1) AS FOLDER_PATH, --substr() removes the file name and trailing \ from the file path by calculating: (length of file name) - (length of file path) - 1 
	NAME, 
	...
	coalesce(SIZE, 0) AS SIZE, --coalesce() replaces any null value in the size column with '0' - makes calculations possible for folders with files with errors/no file size
	...
	FROM amemm_siegfried_202006) --name of table to analyse
